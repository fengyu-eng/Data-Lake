...
// 类名对应退单表：ods_order_refund_info_full
public class ods_order_refund_info_full {
    public static void main(String[] args) throws ExecutionException, InterruptedException {
        ...
        String cdcSql = "CREATE TABLE order_refund_info_full_mq (\n" +
        ...
        String paimonDDl = "CREATE TABLE IF NOT EXISTS ods.ods_order_refund_info_full(\n" +
                "    `id` bigint NOT NULL COMMENT '编号',\n" + // 修正原注释（原为"购物券编号"）
        ...
        // 导入任务（完整保留分区逻辑和过滤条件）
        String insertSql = "INSERT INTO ods.ods_order_refund_info_full(\n" +
                "    `id`, `k1`, `user_id`, `order_id`, `sku_id`, `refund_type`,\n" +
                "    `refund_num`, `refund_amount`, `refund_reason_type`, `refund_reason_txt`,\n" +
                "    `refund_status`, `create_time`\n" +
                ")\n" +
                "select\n" +
                "    id,\n" +
                "    DATE_FORMAT(create_time, 'yyyy-MM-dd') AS k1,\n" + // 按创建时间天分区
                "    `user_id`, `order_id`, `sku_id`, `refund_type`,\n" +
                "    `refund_num`, `refund_amount`, `refund_reason_type`, `refund_reason_txt`,\n" +
                "    `refund_status`, `create_time`\n" +
                "from default_catalog.default_database.order_refund_info_full_mq\n" +
                "where create_time is not null;"; // 保留过滤条件
        TableResult tableResult = tableEnv.executeSql(insertSql);
        System.out.println("order_refund_info导入任务启动成功");
        tableResult.await();
    }
}
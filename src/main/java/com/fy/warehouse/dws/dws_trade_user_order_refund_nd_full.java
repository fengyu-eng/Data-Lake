...
// 交易域用户粒度退单最近30日汇总表
public class dws_trade_user_order_refund_nd_full {
    public static void main(String[] args) {
        ...
        // 创建paimon表
        String paimonSql = "CREATE TABLE IF NOT EXISTS dws.dws_trade_user_order_refund_nd_full(\n" +
        ...
        // 数据插入
        String insertSql = "insert into dws_trade_user_order_refund_nd_full select\n" +
                "     `user_id`                ,-- BIGINT COMMENT '用户id',\n" +
                "    `k1`                      ,--STRING  COMMENT '分区字段',\n" +
                "    count(order_id)`order_refund_count_30d`  ,--BIGINT COMMENT '最近30日退单次数',\n" +
                "    sum(refund_num)`order_refund_num_30d`    ,--BIGINT COMMENT '最近30日退单商品件数',\n" +
                "    sum(refund_amount)`order_refund_amount_30d` --DECIMAL(16, 2) COMMENT '最近30日退单金额'\n" +
                "from (\n" +
                "    select\n" +
                "        user_id,\n" +
                "        k1,\n" +
        ...
        System.out.println("dws_trade_user_order_refund_nd_full导入任务启动成功");
    }
}